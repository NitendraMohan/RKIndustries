    ...
    try {
        ...
        foreach ($result as $row) {
            $output .= "<tr>
                        ...
                        <td>{$row["pincode"]}</td>
                        <td>" . ($row['status'] == 1 
                        ? "<button class='btn btn-success btn-sm btn_toggle' {$permissions['status']}  data-id={$row['id']} data-status='active' data-dbtable='tbl_branch' style='width:70px;'>Active</button>" 
                        : "<button class='btn btn-secondary btn-sm btn_toggle' {$permissions['status']} data-id={$row['id']} data-status='deactive' data-dbtable='tbl_branch' style='width:70px;'>Deactive</button>") . "</td>
                        <td>
...
if ($_POST['action'] == "search") {
    try {
        ...
        $sql = "SELECT * FROM tbl_branch where branch_name like '%{$search_value}%' or address like '%{$search_value}%' or pincode like '%{$search_value}%' ";
        if($statusSearch!=''){
