...
if ($_POST['action'] == "delete") {
    try {
        ...
        if ($recordId) {
            log_user_action($_SESSION['userid'], $_POST['action'], "tbl_brand", $_POST['id'], $_SESSION["username"], json_encode($oldRecord));
            echo 1;
        } else {
            echo 0;
        }
...
if ($_POST['action'] == "search") {
    try {
        ...
        $sql = "SELECT * FROM tbl_brand where brand_name like '%{$search_value}%'";
        $result = $db->readData($sql);
        print_r($result);
