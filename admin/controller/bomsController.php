...
if ($_POST['action'] == "load") {
    try {
        $sql = "SELECT 
                ...
                GROUP BY 
                    b.id, p.product_name, br.brand_name, u.unit, b.qty, b.image, b.status;";
        ...
        foreach ($result as $row) {
            $output .= "<tr>
                        ...
                        <td>" . ($row['status'] == 1 
                        ? "<button class='btn btn-success btn-sm btn_toggle' data-id={$row['id']} data-status='active' data-dbtable='tbl_bom_product' style='width:70px;'>Active</button>" 
                        : "<button class='btn btn-secondary btn-sm btn_toggle' data-id={$row['id']} data-status='deactive' data-dbtable='tbl_bom_product' style='width:70px;'>Deactive</button>") . "</td>
                        <td>
