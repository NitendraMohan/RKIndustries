    ...
    try {
        ...
        foreach ($result as $row) {
            $output .= "<tr>
                        ...
                        <td>" . ($row['status'] == 1 
                        ? "<button class='btn btn-success btn-sm btn_toggle' {$permissions['status']} data-id={$row['id']} data-status='active' data-dbtable='tbl_taxes' style='width:70px;'>Active</button>" 
                        : "<button class='btn btn-secondary btn-sm btn_toggle' {$permissions['status']} data-id={$row['id']} data-status='deactive' data-dbtable='tbl_taxes' style='width:70px;'>Deactive</button>") . "</td>
                        <td>
...
if ($_POST['action'] == "update") {
    try {
        ...
        $oldRecord = $db->readSingleRecord($sql, $params);
        $sql = "select id from tbl_taxes where tax_name=:taxname and id!={$id}";
        $params = ['taxname' => strtoupper($_POST['taxname'])];
        $result = $db->readSingleRecord($sql, $params);
        if (isset($result)) {
            echo json_encode(array('duplicate' => true));
        } else {
