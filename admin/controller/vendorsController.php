    ...
    try {
        ...
        foreach ($result as $row) {
            $output .= "<tr>
                        ...
                        <td>" . ($row['status'] == 1 
                        ? "<button class='btn btn-success btn-sm btn_toggle' {$permissions['status']} data-id={$row['id']} data-status='active' data-dbtable='tbl_vendors' style='width:70px;'>Active</button>" 
                        : "<button class='btn btn-secondary btn-sm btn_toggle' {$permissions['status']} data-id={$row['id']} data-status='deactive' data-dbtable='tbl_vendors' style='width:70px;'>Deactive</button>") . "</td>
                        <td>
...
if ($_POST['action'] == "delete") {
    try {
        ...
        $sql = "select * from tbl_vendors where id=:id";
        $params = ["id" => $_POST["id"]];
        $oldRecord = $db->readSingleRecord($sql, $params);
        $sql = "delete from tbl_vendors where id =:id";
        $params = ['id' => $id];
        $recordId = $db->ManageData($sql, $params);
        if ($recordId) {
            log_user_action($_SESSION['userid'], $_POST['action'], "tbl_vendors", $_POST['id'], $_SESSION["username"], json_encode($oldRecord));
            echo 1;
        } else {
            echo 0;
        }
...
if ($_POST['action'] == "search") {
    try {
        ...
        $sql = "SELECT * FROM tbl_vendors where vendor_name like '%{$search_value}%' or comp_name like '%{$search_value}%' or gstno like '%{$search_value}%' or mobile like '%{$search_value}%' or address like '%{$search_value}%' or email like '%{$search_value}%'";
        if($statusSearch!=''){
