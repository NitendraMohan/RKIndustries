    ...
    try {
        ...
        foreach ($result as $row) {
            $output .= "<tr>
                        ...
                        <td>" . ($row['status'] == 1 
                        ? "<button class='btn btn-success btn-sm btn_toggle' {$permissions['status']} data-id={$row['id']} data-status='active' data-dbtable='tbl_parties' style='width:70px;'>Active</button>" 
                        : "<button class='btn btn-secondary btn-sm btn_toggle' {$permissions['status']} data-id={$row['id']} data-status='deactive' data-dbtable='tbl_parties' style='width:70px;'>Deactive</button>") . "</td>
                        <td>
...
if ($_POST['action'] == "update") {
    try {
        ...
        $sql = "select mobile from tbl_parties where party_name=:partyname and id!={$id}";
        $params = ['partyname' => $partyname];
        $result = $db->readSingleRecord($sql, $params);
        if (isset($result)) {
            echo json_encode(array('duplicate' => true));
